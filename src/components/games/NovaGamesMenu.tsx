...
const GAMES = [
  { id: "roulette", name: "Premium Roulette", nameAr: "الروليت", emoji: "🎰", desc: "ارقام، ألوان، زوجي/فردي - مضاعفات حتى x36", color: "from-yellow-600/20 to-red-900/20", border: "border-yellow-500/30" },
  { id: "liontiger", name: "Lion vs Tiger", nameAr: "أسد ضد نمر", emoji: "🦁", desc: "X2 للأسد/النمر - X30 للتعادل", color: "from-purple-600/20 to-blue-900/20", border: "border-purple-500/30" },
];
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} exit={{ scale: 0.8 }}
              ...
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-black flex items-center gap-2" style={{ color: "#f5c842", textShadow: "0 0 15px rgba(245,200,66,0.4)" }}>
              ...
              <div className="space-y-3">
                {GAMES.map(g => (
                  <button key={g.id} onClick={() => setPreviewGame(g.id)}
        ...
        {previewGame && selectedGame && !activeGame && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.8, y: 50 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.8, y: 50 }}
              className="w-full max-w-sm p-6 space-y-5 text-center">
              <button onClick={() => setPreviewGame(null)} className="absolute top-4 right-4">
              ...
              <div className="pt-2 space-y-2">
                <button onClick={() => { setActiveGame(previewGame); setPreviewGame(null); }}
